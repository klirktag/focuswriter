...
QString FormatManager::filter(const QString& type)
{
	if (type == "odt") {
		...
		return tr("Markdown") + QLatin1String(" (*.md)");
	} else {
		return QString();
	}
}
...
QStringList FormatManager::filters(const QString& type)
{
	...
	if (!type.isEmpty()) {
		...
		result.prepend(tr("All Supported Files") + QLatin1String(" (*.docx *.fodt *.md *.odt *.rtf *.txt *.text)"));
	}
	return result;
}
...
bool FormatManager::isRichText(const QString& filename)
{
	...
	return (type == "odt") || (type == "fodt") || (type == "docx") || (type == "rtf") || (type == "md");
}
